...
export async function multiStepConversation(conversation: Conversation<MyContext>, ctx: MyContext) {
    ...
    // If language is not set in session, default to language selection
        ...
        try {
            switch (currentStep) {
                case ConversationStepsEnum.LANGUAGE_SELECTION:
                    ctx.session.language = await handleLanguageSelection(conversation, ctx);
                    currentStep = user ? ConversationStepsEnum.MAIN_MENU : ConversationStepsEnum.ASKING_PHONE;
                    break;
