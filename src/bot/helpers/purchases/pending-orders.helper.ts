...
import {getConfirmedOrders, getPendingOrders} from "../../services/orders.service";
...
import {paymentsMenu} from "../../controllers/payments-menu";
...
export async function handlePendingOrders(conversation: Conversation<MyContext>, ctx: MyContext) {
    await ctx.reply(ctx.session.language === 'uz' ? 'Kuting, ma\'lumotlar yuklanmoqda...' : 'Пожалуйста, подождите, данные загружаются...');

    const confirmedOrders: PendingOrdersResponse = await getPendingOrders();
    let user = await User.findOne({telegramId: ctx!.from!.id});
    if (!confirmedOrders || !confirmedOrders.value.length) {
        await ctx.reply(
            ctx.session.language === 'uz'
                ? "Sizda jarayondagi buyurtmalar yo'q."
    ...
    const showPage = (page: number): { message: string; keyboard: InlineKeyboard } => {
        ...
        const message = ctx.session.language === 'uz'
            ? `<b>Jarayondagi buyurtmalar (Sahifa ${page}/${totalPages})</b>\n${items}`
            : `<b>В процессе заказы (Страница ${page}/${totalPages})</b>\n${items}`;
