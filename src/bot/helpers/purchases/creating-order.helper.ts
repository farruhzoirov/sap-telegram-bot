            ...
            : `Страница: ${currentPage + 1}/${totalPages}\nВыберите товар:\n${itemList}`,
        {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{text: `✅  ${ctx.session.language === LanguageEnum.uz ? "Tanlash" : "Выбирать"}`, callback_data: "select"}],
                    [{text: `✅ ${ctx.session.language === LanguageEnum.uz ? "Savdoni to'xtatish" : "Прекращение торговли"}`, callback_data: "finish"}]
                ],
            },
        }
    );
